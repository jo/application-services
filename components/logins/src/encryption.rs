...
#[cfg(feature = "keydb")]
use nss::pk11::sym_key::{get_or_create_aes256_key, authentication_with_primary_password_is_needed, authenticate_with_primary_password};
#[cfg(feature = "keydb")]
use std::path::Path;
...
impl KeyManager for NSSKeyManager {
    ...
    fn get_key(&self) -> ApiResult<Vec<u8>> {
        ...
        let key = get_or_create_aes256_key(KEY_NAME)
            .expect("Could not get or create key via NSS");
